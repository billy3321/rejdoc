...
function getReJDocString(input) {
    ...
    this.caseMain       = this.rawData[1].split(regexEndDot).filter(function(el){return el!='';}),
    ...
    this.caseBodyReason = this.caseBodyColumns[0].split(regexFootMarks).filter(function(el){return el!='';}),
    ...
    for (var caseHeadIndex = 0; caseHeadIndex < this.caseHeadLength; caseHeadIndex++) {
        ...
        // ex. 友善列印
        if(regexNavText.test(_term)){continue;}
        ...
        if (regexSummray.test(_term)) {
            ...
            if (regexCaseName.test(_term)) {
                ...
                // ex. ＯＯＯ法院ＯＯ判決(裁定)
                if (regexCaseCourt.test(_term))  {this.caseContent['court']  = _term;}
                // ex. 000年度ＯＯ字第00號
                if (regexCaseNumber.test(_term)) {this.caseContent['case_number'].push(_term);}
            }
            ...
            if (regexParities.test(_term)) {
                var _temp     = _term.match(regexParities),
                    lastParty = _temp[1];
                casePartyGroup+=1;
                if(typeof this.caseContent['case_parities'][casePartyGroup]==='undefined') this.caseContent['case_parities'][casePartyGroup]=[];
                ...
                // ex.（現於臺灣ＯＯ監獄執行中，
                if(regexOpenMarks.test(_term)){caseParyDesc.push(_term);_term='';}
                // ex. 暫寄押臺灣ＯＯ監獄）
    ...
    for (var footerIndex = 0; footerIndex < this.caseFooterLength; footerIndex++) {
        ...
        // avoid ASCII tables
        if(regexASCIITable.test(this.caseFooter[footerIndex])){this.caseContent['fulltext'].push(this.caseFooter[footerIndex]);continue;}
        ...
        if(regexAllMarks.test(_term)){
            ...
            if(regexLawArticle.test(_term)){this.caseContent['fulltext'].push(_term);continue;}
            ...
            // 中華民國yy年mm月dd日
            if(regexFormalDate.test(_term)){this.caseContent['fulltext'].push(_term);continue;}
            ...
            if(regexOfficeChief.test(_term)){
                ...
                this.caseContent['fulltext'].push(_term);continue;
            } else {
                if(regexOffice.test(_term)){this.caseContent['court_div']=_term;}
                if(regexOfficials.test(_term)){this.caseContent['court_member'].push(_term);}
                this.caseContent['fulltext'].push(_term);continue;
            }
        }
...
getReJDocString.prototype.getText = function (){
    ...
    if(isWin) _output = _output.replace(regexLineBreak, '\n\r');
    return _output;
};
